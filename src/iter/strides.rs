...
pub(crate) struct NdIterExtensionStridesPtr(NdIterExtensionStridesPtrMut);

impl NdIterExtensionStridesPtr {
    ...
    pub fn new(strides: &[usize], initial_ptr: *const u8) -> Self {
        Self(NdIterExtensionStridesPtrMut::new(
            strides,
            initial_ptr.cast_mut(),
        ))
    }
}
impl<Ix> NdIterExtension<Ix> for NdIterExtensionStridesPtr
where
    Ix: Idx,
{
    ...
    fn next(&self) -> *const u8 {
        <NdIterExtensionStridesPtrMut as NdIterExtension<Ix>>::next(&self.0).cast_const()
    }
}
...
/// On each dimension change the pointer is adjusted by the difference in byte offsets:
/// `ptr += (after - before) * stride[dim]`.
pub(crate) struct NdIterExtensionStridesPtrMut {
    strides: DimArray<usize>,
    current_ptr: *mut u8,
}

impl NdIterExtensionStridesPtrMut {
    /// Creates the extension starting at `initial_ptr` with the given per-dimension byte strides.
...
impl<Ix> NdIterExtension<Ix> for NdIterExtensionStridesPtrMut
where
    Ix: Idx,
{
    type Item<'a> = *mut u8;
...
mod tests {
    ...
    fn strides_ptr_mut_1d_stride_1() {
        ...
        let ext = NdIterExtensionStridesPtrMut::new(&[1], base);
    ...
    fn strides_ptr_mut_1d_larger_stride() {
        ...
        let base = data.as_mut_ptr();
        let stride = 8usize;
        let ext = NdIterExtensionStridesPtrMut::new(&[stride], base);
        let mut iter = NdIter::new(&[8usize], ext);
        let mut i = 0usize;
        while let Some((_, ptr)) = iter.next() {
    ...
    fn strides_ptr_mut_2d_row_major_contiguous() {
        ...
        let base = data.as_mut_ptr();
        let ext = NdIterExtensionStridesPtrMut::new(&[cols * elem, elem], base);
    ...
    fn strides_ptr_mut_2d_non_contiguous_column_major() {
        ...
        let ext = NdIterExtensionStridesPtrMut::new(&[1, rows], base);
        let mut iter = NdIter::new(&[rows, cols], ext);
    ...
    fn strides_ptr_mut_with_begin_offset() {
        ...
        let ext = NdIterExtensionStridesPtrMut::new(&[4, 1], start);
    ...
    fn strides_ptr_mut_empty_range_yields_no_pointers() {
        let mut data = [0u8; 16];
        let ext = NdIterExtensionStridesPtrMut::new(&[4, 1], data.as_mut_ptr());
    ...
    fn strides_ptr_const_1d_matches_mut() {
        ...
        let ext = NdIterExtensionStridesPtr::new(&[1], base);
        let mut iter = NdIter::new(&[8usize], ext);
        let mut i = 0usize;
        while let Some((_, ptr)) = iter.next() {
    ...
    fn strides_ptr_const_2d_contiguous() {
        ...
        let base = data.as_ptr();
        let ext = NdIterExtensionStridesPtr::new(&[cols, 1], base);
        let mut iter = NdIter::new(&[rows, cols], ext);
        let mut flat = 0usize;
        while let Some((_, ptr)) = iter.next() {
