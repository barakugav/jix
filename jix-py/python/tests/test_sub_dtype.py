"""
Use-case tests for jix.dtype_sub_field.
"""
...
def _struct_1d():
    """Return a 1-D numpy structured array and its jix counterpart."""
    dt = np.dtype([("x", np.int32), ("y", np.int32)])
    pts = np.array([(1, 10), (2, 20), (3, 30)], dtype=dt)
    return pts, jix.asarray(pts)
...
def test_mixed_field_dtypes():
    ...
    a = jix.dtype_sub_field(za, "a")
    b = jix.dtype_sub_field(za, "b")
...
def test_compact_array_input():
    """dtype_sub_field also works when the input is a jix compact array."""
    dt = np.dtype([("x", np.int32), ("y", np.int32)])
