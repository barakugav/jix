...
def _ref_isclose_complex_scalar(a, b, rtol, atol_re, atol_im):
    ...
    re_type = type(a.real)
    return _ref_isclose_scalar(re_type(a.real), re_type(b.real), rtol, atol_re) and \
           _ref_isclose_scalar(re_type(a.imag), re_type(b.imag), rtol, atol_im)
...
def _ref_isclose_array(np_a, np_b, rtol, atol, dtype):
    ...
    b_t = np_b.astype(ft)
    return np.vectorize(
        lambda a, b: _ref_isclose_scalar(a, b, rtol_t, atol_t), otypes=[bool]
    )(a_t, b_t)
