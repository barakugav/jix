...
def test_asarray_from_zix_array_is_noop():
    arr = zix.compact([1, 2, 3], dtype=np.int32)
    a = zix.asarray(arr)
    np.testing.assert_array_equal(a.numpy(), [1, 2, 3])
...
def test_astype_scalar_casts(src_dtype, dst_dtype):
    ...
    za = zix.compact(data)
    result = zix.astype(za, dst_dtype)
...
def test_astype_preserves_shape():
    ...
    za = zix.compact(arr)
    result = zix.astype(za, np.float64)
...
def test_flatten_lazy():
    ...
    za = zix.compact(arr)
    flat = zix.flatten(za, copy=False)
    assert flat.shape == (6,)
    np.testing.assert_array_equal(flat.numpy(), arr)
...
def test_reshape_lazy():
    arr = np.arange(6, dtype=np.int32)
    za = zix.compact(arr)
    r = zix.reshape(za, [3, 2], copy=False)
    assert r.shape == (3, 2)
...
def test_unsqueeze_same_as_insert_axis():
    arr = np.arange(6, dtype=np.int32).reshape(2, 3)
    za = zix.compact(arr)
    r1 = zix.unsqueeze(za, 1)
    r2 = zix.insert_axis(za, 1)
...
def test_concatenate_axis0():
    a = zix.compact(np.array([[1, 2], [3, 4]], dtype=np.int32))
    b = zix.compact(np.array([[5, 6]], dtype=np.int32))
    r = zix.concatenate([a, b], axis=0)
    assert r.shape == (3, 2)
...
def test_concatenate_axis1():
    a = zix.compact(np.array([[1, 2], [3, 4]], dtype=np.int32))
    b = zix.compact(np.array([[5], [6]], dtype=np.int32))
    r = zix.concatenate([a, b], axis=1)
    assert r.shape == (2, 3)
    np.testing.assert_array_equal(r.numpy(), np.array([[1, 2, 5], [3, 4, 6]]))


def test_concatenate_negative_axis():
    a = zix.compact(np.array([1, 2, 3], dtype=np.int32))
    b = zix.compact(np.array([4, 5], dtype=np.int32))
    r = zix.concatenate([a, b], axis=-1)
    assert r.shape == (5,)
...
def test_concatenate_three_arrays():
    arrays = [zix.compact(np.array([i, i + 1], dtype=np.float32)) for i in range(3)]
    r = zix.concatenate(arrays, axis=0)
    assert r.shape == (6,)


def test_concatenate_default_axis():
    a = zix.compact(np.array([1, 2], dtype=np.int32))
    b = zix.compact(np.array([3, 4], dtype=np.int32))
    r = zix.concatenate([a, b])
    np.testing.assert_array_equal(r.numpy(), [1, 2, 3, 4])


def test_concatenate_dtype_mismatch_raises():
    a = zix.compact(np.array([1, 2], dtype=np.int32))
    b = zix.compact(np.array([3.0, 4.0], dtype=np.float32))
    with pytest.raises(Exception):
        zix.concatenate([a, b])
...
def test_stack_axis0():
    a = zix.compact(np.array([1, 2, 3], dtype=np.int32))
    b = zix.compact(np.array([4, 5, 6], dtype=np.int32))
    r = zix.stack([a, b], axis=0)
    assert r.shape == (2, 3)
    np.testing.assert_array_equal(r.numpy(), np.array([[1, 2, 3], [4, 5, 6]]))
...
def test_stack_2d_arrays():
    a = zix.compact(np.zeros((2, 3), dtype=np.float32))
    b = zix.compact(np.ones((2, 3), dtype=np.float32))
    r = zix.stack([a, b], axis=0)
...
def test_stack_shape_mismatch_raises():
    a = zix.compact(np.array([1, 2], dtype=np.int32))
    b = zix.compact(np.array([3, 4, 5], dtype=np.int32))
    with pytest.raises(Exception):
        zix.stack([a, b])
...
def test_where_basic():
    cond = zix.compact(np.array([True, False, True, False], dtype=bool))
    x = zix.compact(np.array([1, 2, 3, 4], dtype=np.int32))
    y = zix.compact(np.array([10, 20, 30, 40], dtype=np.int32))
    r = zix.where(cond, x, y)
    np.testing.assert_array_equal(r.numpy(), [1, 20, 3, 40])


def test_where_float():
    cond = zix.compact(np.array([True, False, True], dtype=bool))
    x = zix.compact(np.array([1.0, 2.0, 3.0], dtype=np.float32))
    y = zix.compact(np.array([0.1, 0.2, 0.3], dtype=np.float32))
    r = zix.where(cond, x, y)
...
def test_where_matches_numpy():
    ...
    r = zix.where(zix.compact(cond), zix.compact(x), zix.compact(y))
...
def test_write_and_read_array_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        path = Path(tmpdir) / "test.zix"
        zix.write_array(za, path)
        loaded = zix.read_array(path)
...
def test_read_array_mmap():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        path = Path(tmpdir) / "test.zix"
        zix.write_array(za, path)
        loaded = zix.read_array(path, mmap=True)
        np.testing.assert_array_equal(loaded.numpy(), arr)
