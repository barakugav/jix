...
def make_array(values, dtype=np.float32):
    return zix.compact(np.array(values, dtype=dtype))
...
def test_write_array_path_fails_if_exists():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with pytest.raises(Exception):
            zix.write_array(a, p)  # must not exist by default
...
def test_write_array_path_append():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        zix.write_array(b, p, append=True)

        a2 = zix.read_array(p)
        b2 = zix.read_array(p, offset=offset)
...
def test_read_array_path_packed(tmp_path):
    a = make_array([1.0, 2.0, 3.0])
    b = make_array([10, 20, 30], dtype=np.int32)
    p = tmp_path / "packed.zix"
    zix.write_array(a, p)
    offset = p.stat().st_size
    zix.write_array(b, p, append=True)
    total = p.stat().st_size

    a2 = zix.read_array(p, offset=0, len=offset)
    b2 = zix.read_array(p, offset=offset, len=total - offset)
    np.testing.assert_array_equal(a2.numpy(), a.numpy())
    np.testing.assert_array_equal(b2.numpy(), b.numpy())
...
def test_read_array_reader_at_offset():
    ...
    zix.write_array(a, buf)
    offset = buf.tell()
    zix.write_array(b, buf)

    buf.seek(offset)
    b2 = zix.read_array(buf)
    np.testing.assert_array_equal(b2.numpy(), b.numpy())
